...
function CategorizedAccountList(props) {
    const renderCategorizedList = (accounts, categoryList) => {
        const entries = [];
        ...
        const mapEntry = function (category, prefix) {
            ...
            const categoryList = <AccountList key={'accountlist-' + category.get('id')} actions={props.actions}
                                              currencies={props.currencies} accounts={categoryAccounts}
    ...
    const categorizedAccounts = renderCategorizedList(filteredAccounts, categories);
    ...
    return (
        <>
            <AccountList actions={props.actions} currencies={props.currencies} accounts={simpleAccounts}
                         hiddenVisible={props.hiddenVisible}/>
            {categorizedAccounts}
        </>
    )
