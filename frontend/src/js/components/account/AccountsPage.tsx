...
function AccountsPage(props: AccountsPageProps) {
    const emptyAccount: Account = {
        ...
        account_type: 'ASSET',
        balance: 0,
        currency_id: props.primaryCurrencyId,
        operational: false,
        favorite: false,
        primary_balance: 0,
    };
    ...
    if (showHidden) {
        hiddenButton = (
            <Button color="primary" onClick={() => setShowHidden(false)}>
                Hide hidden accounts
            </Button>
        );
    } else {
        hiddenButton = (
            <Button color="primary" onClick={() => setShowHidden(true)}>
        ...
        <Fragment>
                ...
                <CardContent>
                    <Grid container spacing={2}>
                        <Grid item xs={12} sm={12} md={6} lg={4}>
                            <p>
                                Total: {props.totals.total} {props.primaryCurrencyName}
                            </p>
                        </Grid>
                        <Grid item xs={6} sm={6} md={6} lg={4} className="hide-on-small">
                            <p>
                                Favorite: {props.totals.favorite} {props.primaryCurrencyName}
                            </p>
                        </Grid>
                        <Grid item xs={6} sm={6} md={4} lg={4} className="hide-on-small hide-on-medium">
                            <p>
                                Operational: {props.totals.operational} {props.primaryCurrencyName}
                            </p>
                        </Grid>
                        ...
                        <Grid item xs={12} sm={12} md={6} lg={3}>
                            <Button aria-label="Add account" color="secondary" onClick={onCreateAccountClick}>
                                Add account
                            </Button>
                        </Grid>
                    </Grid>
            ...
            {tabValue == 'asset' && (
                <CategorizedAccountList
                    tree={props.assetAccountsTree}
                    indent={0}
                    hidden={showHidden}
                    accountEdit={onEditAccountClick}
                />
            )}
            {tabValue == 'income' && (
                <CategorizedAccountList
                    tree={props.incomeAccountsTree}
                    indent={0}
                    hidden={showHidden}
                    accountEdit={onEditAccountClick}
                />
            ...
            {tabValue == 'expense' && (
                <CategorizedAccountList
                    tree={props.expenseAccountsTree}
                    indent={0}
                    hidden={showHidden}
                    accountEdit={onEditAccountClick}
                />
