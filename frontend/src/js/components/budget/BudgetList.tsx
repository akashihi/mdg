...
export function FormikDatePicker(props: FieldAttributes<any>) {
    ...
    return <DatePicker name={props.field.name} value={props.field.value} format='d/M/yyyy' onChange={v => setFieldValue(props.field.name, v)} />
}
...
export function BudgetList(props: BudgetSelectorProps) {
    ...
    const budgetList = budgets.map((b:ShortBudget, index:number) => <MenuItem key={index} value={b.id}>{`${b.term_beginning} - ${b.term_end}`}</MenuItem>);
    ...
    return <Fragment>
        ...
        Select budget:
        <Select disabled={budgets.length === 0} value={props.selectedBudgetId} onChange={(ev) => props.loadSelectedBudget(ev.target.value)}>
            {budgetList}
        </Select>
        <Button color='primary' onClick={onDeleteBudget}>Delete selected budget</Button>
