...
export function TransactionsPageFilter(props: TransactionFilterProps) {
    ...
    return (
        <Grid container spacing={2} style={{ height: '340px' }}>
            ...
            <Grid item xs={6} sm={6} md={6} lg={6}>
                <DatePicker
                    format="d/M/yyyy"
                    value={periodBeginning.toDate()}
                    onChange={v => {
                        if (v !== null) {
                            setPeriodBeginning(moment(v));
                        } else {
                            setPeriodBeginning(moment().subtract(1, 'month'));
                        }
                    }}
                />
            </Grid>
            ...
            <Grid item xs={6} sm={6} md={6} lg={6}>
                <DatePicker
                    format="d/M/yyyy"
                    value={periodEnd.toDate()}
                    onChange={v => {
                        if (v !== null) {
                            setPeriodEnd(moment(v));
                        } else {
                            setPeriodEnd(moment());
                        }
                    }}
            ...
            <Grid item xs={12} sm={6} md={6} lg={6}>
                <Button variant="text" sx={buttonStyle} onClick={() => setPeriodDays(0)}>
                    Today
                </Button>
                <Button variant="text" sx={buttonStyle} onClick={() => setPeriodDays(7)}>
                    Week
                </Button>
                <Button variant="text" sx={buttonStyle} onClick={() => setPeriodDays(30)}>
                    Month
                </Button>
                <Button variant="text" sx={buttonStyle} onClick={() => setPeriodDays(90)}>
                    Three months
                </Button>
                <Button variant="text" sx={buttonStyle} onClick={() => setPeriodDays(365)}>
                    Year
                </Button>
            </Grid>
            <Grid item xs={12} sm={6} md={6} lg={6}>
            ...
            <Grid item xs={4} sm={4} md={4} lg={4}>
                <Button aria-label="Done" onClick={applyFilter}>
                    <Done />
                </Button>
                <Button aria-label="Clear" onClick={clearFilter}>
                    <Clear />
                </Button>
            </Grid>
        </Grid>
