...
export function TransactionDeleteConfirmation(props: TransactionDeleteProps) {
    ...
    return <Dialog open={props.visible}>
        <DialogContent>
          Please confirm transaction '{name}' deletion
        </DialogContent>
        <DialogActions>
          <Button key='cancel-button' color='primary' onClick={props.close} >Cancel</Button>
          <Button key='delete-button' color='secondary' onClick={() => props.delete(props.tx)}>Delete</Button>
        </DialogActions>
      </Dialog>
}
