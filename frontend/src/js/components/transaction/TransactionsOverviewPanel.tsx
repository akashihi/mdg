...
function TransactionShortWidget(props:EnrichedTransaction) {
        return (
            <Grid container spacing={2}>
                <Grid item xs={3} sm={2} md={1} lg={3}>{props.timestamp}</Grid>
                <Grid item xs={6} sm={3} md={3} lg={3}>{props.comment}</Grid>
                ...
                <Grid item xs={1} sm={3} md={2} lg={2} className='hide-on-small'>{props.tags.join(', ')}</Grid>
            </Grid>
        )
}

export function TransactionsOverviewPanel(props: TransactionOverviewProps) {
    useEffect(() => {props.loadLastTransactions()}, []);
    const transactions = props.transactions.map((item, id) => <ListItem key={id}><TransactionShortWidget {...item} /></ListItem>);
