...
/**
  * Represents default values for different types.
  *
  * Usage: Default.value[Float]
  */
...
object Default extends LowerPriorityImplicits {
  ...
  implicit def defaultMap[A, B]: Default[Map[A, B]] =
    new Default[Map[A, B]](Map[A, B]())
  implicit def defaultOption[A]: Default[Option[A]] =
    new Default[Option[A]](None)
