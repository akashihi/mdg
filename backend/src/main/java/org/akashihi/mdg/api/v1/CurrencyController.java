...
public class CurrencyController {
    ...
    Currency get(@PathVariable("id") Long id) {
        return currencyService.get(id).orElseThrow(() -> new MdgException("CURRENCY_NOT_FOUND"));
    }
    ...
    Currency update(@PathVariable("id") Long id, @RequestBody Currency currency) {
        return currencyService.update(id, currency).orElseThrow(() -> new MdgException("CURRENCY_NOT_FOUND"));
    }
}
