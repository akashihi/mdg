...
public interface AccountRepository extends JpaRepository<Account, Long>, JpaSpecificationExecutor<Account> {
    ...
    @Query(nativeQuery = true, value = "select sum(o.amount) as amount, sum(o.amount*coalesce(r.rate,1)) as primary_amount, c.code as name from operation as o left outer join account as a on (o.account_id = a.id) inner join tx on (o.tx_id = tx.id) inner join currency c on c.id = a.currency_id inner join setting as s on (s.name='currency.primary') left outer join rates r on c.id = r.from_id and r.to_id=s.value\\:\\:bigint where a.account_type = 'asset' and tx.ts < ?1 and r.rate_beginning <= ?1 and r.rate_end > ?1 group by c.code;")
    List<AmountAndName> getTotalAssetsForDateByCurrency(LocalDate dt);

    @Query(nativeQuery = true, value = "select sum(o.amount) as amount, sum(o.amount*coalesce(r.rate,1)) as primary_amount, c.name as name from operation as o left outer join account as a on(o.account_id = a.id) inner join tx on (o.tx_id=tx.id) inner join setting as s on (s.name='currency.primary') inner join category c on c.id = a.category_id left outer join rates as r on (r.from_id=a.currency_id and r.to_id=s.value\\:\\:bigint and r.rate_beginning <= now() and r.rate_end > now()) where a.account_type='asset' and tx.ts < ?1 group by c.name")
    List<AmountAndName> getTotalAssetsForDateByType(LocalDate dt);

    @Query(nativeQuery = true, value = "select sum(o.amount) as amount, sum(o.amount*coalesce(r.rate,1)) as primary_amount, a.name as name from operation as o left outer join account as a on(o.account_id = a.id) inner join tx on (o.tx_id=tx.id) inner join setting as s on (s.name='currency.primary') left outer join rates as r on (r.from_id=a.currency_id and r.to_id=s.value\\:\\:bigint and r.rate_beginning <= now() and r.rate_end > now()) where a.account_type=?1 and tx.ts between ?2 and ?3 group by a.name order by a.name")
