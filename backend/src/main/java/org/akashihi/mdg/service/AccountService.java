...
public class AccountService {
    ...
    public Account create(Account account) {
        if (!account.getAccountType().equals(AccountType.ASSET)) {
            if (account.getOperational() != null && account.getOperational()) {
                throw new MdgException("ACCOUNT_NONASSET_INVALIDFLAG");
            }
        ...
        var currency = currencyRepository.findById(account.getCurrencyId()).orElseThrow(() ->new MdgException("CURRENCY_NOT_FOUND"));
        account.setCurrency(currency);
        if (account.getCategoryId() != null) {
            var category = categoryRepository.findById(account.getCategoryId()).orElseThrow(() ->new MdgException("CATEGORY_NOT_FOUND"));
    ...
    public Optional<Account> update(Long id, Account newAccount) {
        ...
        if (newAccount.getCategoryId() == null) {
            ...
            if (!newAccount.getCategoryId().equals(currentCategoryId)) {
                var newCategory = categoryRepository.findById(newAccount.getCategoryId()).orElseThrow(() ->new MdgException("CATEGORY_NOT_FOUND"));
        ...
        if (account.getAccountType() == AccountType.ASSET) {
            ...
            if (!account.getCurrency().getId().equals(newAccount.getCurrencyId())) {
                throw new MdgException("ACCOUNT_CURRENCY_ASSET");
            }
        } else {
            if (!account.getCurrency().getId().equals(newAccount.getCurrencyId())) {
    ...
    public void delete(Long id) {
        var account = accountRepository.findById(id).orElseThrow(() -> new MdgException("ACCOUNT_NOT_FOUND"));
        if (!isDeletable(account.getId())) {
            throw new MdgException("ACCOUNT_IN_USE");
        }
