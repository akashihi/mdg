...
class RateController(private val rateService: RateService) {
    ...
    fun listForTs(@PathVariable("ts") @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) ts: LocalDateTime): Rates = Rates(rateService.listForTs(ts))

    @GetMapping(value = ["/rates/{ts}/{from}/{to}"], produces = ["application/vnd.mdg+json;version=1"])
    fun pairForTs(@PathVariable("ts") @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) ts: LocalDateTime, @PathVariable("from") from: Long, @PathVariable("to") to: Long): Rate = rateService.getPair(ts, from, to)
}